...
// RUN: circt-opt %t.pre.mlir -pass-pipeline='builtin.module(hw.module(synth-exact-synthesis{kind=mig objective=depth-size sat-solver=z3}))' -o %t.db.mlir
// RUN: circt-opt %s -pass-pipeline='builtin.module(hw.module(synth-cut-rewrite{db-files=%t.pre.mlir,%t.db.mlir}))' -o %t.mlir

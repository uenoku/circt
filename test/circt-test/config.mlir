...
// RUN: circt-test --list-runners -c %S/config.json %s | FileCheck %s --check-prefix=ADDED
// ADDED-DAG: custom  formal
// ADDED-DAG: sby  formal

// Multiple -c options aggregate runners.
// RUN: circt-test --list-runners -c %S/config-a.json -c %S/config-b.json %s | FileCheck %s --check-prefix=MULTI
// MULTI-DAG: runA  formal
// MULTI-DAG: runB  simulation
// MULTI-DAG: sby  formal

// useDefaultRunners: false suppresses defaults.
// RUN: circt-test --list-runners -c %S/config-no-defaults.json %s | FileCheck %s --check-prefix=NODEF
// NODEF: only  formal
// NODEF-NOT: sby
// NODEF-NOT: circt-bmc
// NODEF-NOT: verilator
...
// RUN: circt-test --list-runners -c %S/config-dup.json %s | FileCheck %s --check-prefix=OVERWRITE
// OVERWRITE: runA  simulation  unavailable
// OVERWRITE-NOT: runA  formal

// Invalid JSON produces an error.
// RUN: not circt-test --list-runners -c %S/config-bad.json %s 2>&1 | FileCheck %s --check-prefix=BADJSON

// RUN: circt-opt --pass-pipeline='builtin.module(hw.design(export-verilog))'  --split-input-file %s | FileCheck %s
// RUN: circt-opt -test-apply-lowering-options='options=emittedLineLength=10' --pass-pipeline='builtin.module(hw.design(export-verilog))'  --split-input-file %s | FileCheck %s

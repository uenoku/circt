...
"sifive.enterprise.firrtl.ExtractAssertionsAnnotation", directory = "dir3",  filename = "./dir3/filename3" }]}
{
  ...
  firrtl.module @Simple(in %in1: !firrtl.uint<4>,
    ...
    // Test CatPrimOp with multiple operands
    // CHECK: [[VCAT1:%.+]] = comb.concat %in1, %in2, [[PADRES2]] : i4, i2, i4
    %vcat1 = firrtl.cat %in1, %in2, %4 : (!firrtl.uint<4>, !firrtl.uint<2>, !firrtl.uint<4>) -> !firrtl.uint<10>

    // Test CatPrimOp with single operand (should pass through)
    // CHECK: %vcat2 = hw.wire %in1 : i4
    %vcat2 = firrtl.wire : !firrtl.uint<4>
    %vcat2_val = firrtl.cat %in1 : (!firrtl.uint<4>) -> !firrtl.uint<4>

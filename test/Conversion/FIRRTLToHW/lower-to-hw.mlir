...
//    stop(clock2, reset, 0)
  ...
  // CHECK-NEXT: %r = seq.firreg %a clock %clock {sv.attributes = [#sv.attribute<"keep = \22true\22", emitAsComment>]}
  firrtl.module @SVAttr(in %a: !firrtl.uint<1>, in %clock: !firrtl.clock, out %b1: !firrtl.uint<1>, out %b2: !firrtl.uint<1>) attributes {convention = #firrtl<convention scalarized>, sv.attributes = [#sv.attribute<"keep_hierarchy = \22true\22">]} {
    ...
    %r = firrtl.reg %clock {sv.attributes = [#sv.attribute<"keep = \22true\22", emitAsComment>]} : !firrtl.clock, !firrtl.uint<1>

...
firrtl.circuit "InstanceChoiceProbe" {
  ...
  firrtl.module @InstanceChoiceProbe(out %out: !firrtl.uint<8>) {
    %inst_probe = firrtl.instance_choice inst {instance_macro = @__target_Platform_InstanceChoiceProbe_inst} @DefaultTarget alternatives @Platform { @FPGA -> @FPGATarget, @ASIC -> @ASICTarget } (out probe: !firrtl.probe<uint<8>>)
...
// Test XMR lowering through nested instance_choice
  firrtl.circuit "MultipleOptions" {
    ...
    firrtl.module @MultipleOptions(out %out1: !firrtl.uint<2>, out %out2: !firrtl.uint<2>) {
      %inst1_probe = firrtl.instance_choice inst1 {instance_macro = @__target_Platform_MultipleOptions_inst1} @Target1 alternatives @Platform { @FPGA -> @Target1, @ASIC -> @Target1 } (out probe: !firrtl.probe<uint<2>>)
      %inst2_probe = firrtl.instance_choice inst2 {instance_macro = @__target_Optimization_MultipleOptions_inst2} @Target2 alternatives @Optimization { @Fast -> @Target2, @Slow -> @Target2 } (out probe: !firrtl.probe<uint<2>>)

// RUN: circt-opt %s -verify-diagnostics --mlir-print-local-scope
// --mlir-print-debuginfo | circt-opt --mlir-print-local-scope
// --mlir-print-debuginfo -verify-diagnostics | FileCheck %s

// CHECK-LABEL: om.class @Thingy
// CHECK-SAME: (%blue_1: i8, %blue_2: i32) -> (widget: !om.class.type<@Widget>,
// gadget: !om.class.type<@Gadget>, blue_1: i8, blue_2: i8)
om.class @Thingy(% blue_1
                 : i8, % blue_2
                 : i32)
    ->(widget
       : !om.class.type<@Widget>, gadget
       : !om.class.type<@Gadget>, blue_1
       : i8, blue_2
       : i8) {
  ...
  % 0 = om.constant 5 : i8
        ...
        % 1 = om.constant 6 : i32
              // CHECK: %[[widget:.+]] = om.object @Widget(%[[c5]], %[[c6]]) :
              // (i8, i32) -> !om.class.type<@Widget>
              % 2 =
                  om.object @Widget(% 0, % 1)
      : (i8, i32)->!om.class.type<@Widget>
                  ...
                  % 3 =
                      ...
                      % 4 =
                          om.constant 8 : i32
                          // CHECK: %[[gadget:.+]] = om.object @Gadget(%[[c7]],
                          // %[[c8]]) : (i8, i32) -> !om.class.type<@Gadget>
                          % 5 = om.object @Gadget(% 3, % 4)
      : (i8, i32)->!om.class.type<@Gadget>

                                // CHECK: %[[widget_field:.+]] = om.object.field
                                // : (!om.class.type<@Widget>) -> i8
                                % 6 =
                                    om.object.field
      : (!om.class.type<@Widget>)
            ->i8

                // CHECK: om.class.fields {test = "fieldsAttr"} %2, %5, %blue_1,
                // %6 : !om.class.type<@Widget>, !om.class.type<@Gadget>, i8, i8
                // field_locs([loc("loc0"), loc("loc1"), loc("loc2"),
                // loc("loc3")]) loc("test")
                    om.class.fields{test = "fieldsAttr"} %
                                    2,
    % 5, % blue_1, % 6 : !om.class.type<@Widget>, !om.class.type<@Gadget>, i8,
    i8 field_locs([ loc("loc0"), loc("loc1"), loc("loc2"), loc("loc3") ])
        loc("test")
}

// CHECK-LABEL: om.class @Widget
// CHECK-SAME: (%blue_1: i8, %green_1: i32) -> (blue_1: i8, green_1: i32)
om.class @Widget(% blue_1
                 : i8, % green_1
                 : i32)
    ->(blue_1
       : i8, green_1
...
om.class @ExternObject(% param1 : i1, % param2 : i2) {
  ...
  % 0 = om.object @Extern(% param1, % param2)
      : (i1, i2)->!om.class.type<@Extern>

        // CHECK: om.object.field
        % 1 = om.object.field : (!om.class.type<@Extern>)->i3 om.class.fields
}

om.class @NestedField1()
    ->(baz
       : i1){ % 0 = om.constant 1 : i1 om.class.fields % 0 : i1}

om.class @NestedField2()
    ->(bar
       : !om.class.type<@NestedField1>) {
  % 0 = om.object @NestedField1()
      : ()->!om.class.type<@NestedField1> om.class.fields %
        0 : !om.class.type<@NestedField1>
}

om.class @NestedField3()->(foo : !om.class.type<@NestedField2>) {
  % 0 = om.object @NestedField2()
      : ()->!om.class.type<@NestedField2> om.class.fields %
        0 : !om.class.type<@NestedField2>
}
...
om.class @NestedField4() {
  ...
  % 0 =
      om.object @NestedField3()
      : ()->!om.class.type<@NestedField3>
      // CHECK: %[[foo:.+]] = om.object.field %[[nested]][@foo] :
      // (!om.class.type<@NestedField3>) -> !om.class.type<@NestedField2> CHECK:
      // %[[bar:.+]] = om.object.field %[[foo]][@bar] :
      // (!om.class.type<@NestedField2>) -> !om.class.type<@NestedField1> CHECK:
      // %{{.+}} = om.object.field %[[bar]][@baz] :
      // (!om.class.type<@NestedField1>) -> i1
      % 1 =
          om.object.field %
          0 [@foo] : (!om.class.type<@NestedField3>)
                         ->!om.class.type<@NestedField2> %
          2 = om.object.field %
              1 [@bar] : (!om.class.type<@NestedField2>)
                             ->!om.class.type<@NestedField1> %
              3 = om.object.field %
                  2 [@baz] : (!om.class.type<@NestedField1>)->i1 om.class.fields
}
...
om.class @ReferenceParameter(% arg0
                             : !om.ref, % arg1
                             : !om.sym_ref)
    ->(myref
       : !om.ref, sym
...
om.class @ReferenceConstant()
  ...
  % 0 = om.constant #om.ref<#hw.innerNameRef<@A::@inst_1>> : !om.ref
        ...
        % 1 = om.constant #om.sym_ref<@A>
      : !om.sym_ref

            // CHECK: om.class.fields %[[const1]], %[[const2]] : !om.ref,
            // !om.sym_ref
                om.class.fields %
              0,
    % 1 : !om.ref, !om.sym_ref
...
om.class @ListConstant()
    ->(list_i64
       : !om.list<i64>, list_i32
       : !om.list<i32>){
      // CHECK: %[[const1:.+]] = om.constant #om.list<i64, [42]> : !om.list<i64>
      % 0 = om.constant #om.list<i64, [42]> : !om.list<i64>

            // CHECK: %[[const2:.+]] = om.constant #om.list<i32, []> :
            // !om.list<i32>
            % 1 = om.constant #om.list<i32, []> :
                      !om.list<i32>

                          // CHECK: om.class.fields %[[const1]], %[[const2]] :
                          // !om.list<i64>, !om.list<i32>
                              om.class.fields %
                  0,
      % 1 : !om.list<i64>,
      !om.list<i32>
      ...
      % cst6_i8 =
          ...
          % cst5_i32 =
              ...
              % cst6_i32 =
                  ...
                  // CHECK: [[obj0:%.+]] = om.object @Widget([[cst5_i8]],
                  // [[cst6_i32]]) : (i8, i32) -> !om.class.type<@Widget>
                  % obj0 =
                      om.object @Widget(% cst5_i8, % cst6_i32)
      : (i8, i32)->!om.class.type<@Widget>

                      // CHECK: [[obj1:%.+]] = om.object @Widget([[cst6_i8]],
                      // [[cst5_i32]]) : (i8, i32) -> !om.class.type<@Widget>
                      % obj1 = om.object @Widget(% cst6_i8, % cst5_i32)
      : (i8, i32)->!om.class.type<@Widget>

                               // CHECK: [[list:%.+]] = om.list_create [[obj0]],
                               // [[obj1]] : !om.class.type<@Widget>
                               % list = om.list_create % obj0,
    %
        obj1 : !om.class
                   .type<@Widget>

                       // CHECK: om.class.fields [[list]] :
                       // !om.list<!om.class.type<@Widget>>
                           om.class.fields %
        list : !om.list<!om.class.type<@Widget>>
}

// CHECK-LABEL: @ListConcat
om.class @ListConcat(){
  % 0 = om.constant #om.integer < 0 : i8 > : !om.integer % 1 =
            om.constant #om.integer < 1 : i8 > : !om.integer % 2 =
                om.constant #om.integer <
                2 : i8 > : !om.integer
...
om.class @IntegerConstant()
    ->(int
       : !om.integer){
      // CHECK: %[[const1:.+]] = om.constant #om.integer<36755551979133953793 :
      // i67> : !om.integer
      % 0 = om.constant #om.integer <
            36755551979133953793 : i67 > : !om.integer
...
om.class @PropEq(){
  ...
  % 1 : !om.string
      ...
      % 3 = om.constant #om.integer < 1 : i8 > : !om.integer % 4 =
                om.constant #om.integer <
                2 : i8 > : !om.integer
  ...
  % 4 : !om.integer
      ...
      % 6 = om.constant true % 7 = om.constant false
...
om.class @LinkedList(% prev
                     : !om.class.type<@LinkedList>)
    ->(prev
       : !om.class.type<@LinkedList>) {
...
om.class @ReferenceEachOther() {
  // CHECK-NEXT: %[[obj1:.+]] = om.object @LinkedList(%[[obj2:.+]]) :
  // (!om.class.type<@LinkedList>) -> !om.class.type<@LinkedList> CHECK-NEXT:
  // %[[obj2]] = om.object @LinkedList(%[[obj1]]) :
  // (!om.class.type<@LinkedList>) -> !om.class.type<@LinkedList>
  % 0 = om.object @LinkedList(% 1)
      : (!om.class.type<@LinkedList>)->!om.class.type<@LinkedList> %
        1 = om.object @LinkedList(% 0)
      : (!om.class.type<@LinkedList>)
            ->!om.class.type<@LinkedList>
                om.class.fields
}
...
om.class @RefecenceEachOthersField(% blue_1 : i8, % green_1 : i32) {
  // CHECK-NEXT: %[[obj1:.+]] = om.object @Widget(%blue_1, %[[field2:.+]]) :
  // (i8, i32) -> !om.class.type<@Widget>
...
om.class @BoolConstant(% b0
    ...
    ->(bool
      ...
      % 1 = om.constant true
            ...
            % 2 = om.constant false
                  // CHECK: om.class.fields %b0, %[[const1]], %[[const2]] : i1,
                  // i1, i1
                  om.class.fields %
                  b0,
      % 1,
...
hw.hierpath @HierPath[@PathModule::@wire] hw.module @PathModule() {
...
om.class @FrozenPath(% basepath
                     : !om.frozenbasepath){
    // CHECK: %[[v0:.+]] = om.frozenbasepath_create %basepath "Foo/bar"
    % 0 = om.frozenbasepath_create %
          basepath "Foo/bar"
          // CHECK: %[[v1:.+]] = om.frozenpath_create reference %basepath
          // "Foo/bar:Bar>w.a"
          % 1 = om.frozenpath_create reference %
                basepath "Foo/bar:Bar>w.a" om.class.fields}
...
om.class @IntegerArithmetic(){
  % 0 = om.constant #om.integer < 1 : si3 > : !om.integer % 1 =
            om.constant #om.integer <
            2 : si3 > : !om.integer
...
om.class @UnknownValue() {
  ...
  % 0 = om.unknown : i8
        ...
        // CHECK-NEXT: %1 = om.object @UnknownValueObject(%0) : (i8) ->
        // !om.class.type<@UnknownValueObject>
        % 1 = om.object @UnknownValueObject(% 0)
      : (i8)->!om.class
...
om.class @PropertyAssert(% basepath : !om.basepath, % cond : i1) {
  ...
  om.property_assert % cond, "invariant must hold" : i1 om.class.fields
}

// RUN: circt-opt --pass-pipeline='builtin.module(hw.module(synth-test-priority-cuts{max-cut-input-size=3 max-cuts-per-root=8}))' %s --mlir-disable-threading | FileCheck %s

// This test exercises choice cuts whose alternative operands expose different
// internal leaves. Truth-table computation for copied choice cuts must use the
// selected operand cut's leaves, not resimulate through the first choice
// operand.
//

...
hw.module @Basic(in %a: i2, in %b: i2, out f: i2) {
  ...
  // CHECK-DAG: %[[AND_INV_0:.+]] = aig.and_inv not %[[EXTRACT_A_1]], %[[EXTRACT_B_1]]
  // CHECK-DAG: %[[EXTRACT_A_0:.+]] = comb.extract %a from 0
  // CHECK-DAG: %[[EXTRACT_B_0:.+]] = comb.extract %b from 0
  // CHECK-DAG: %[[AND_INV_1:.+]] = aig.and_inv not %[[EXTRACT_A_0]], %[[EXTRACT_B_0]]
  // CHECK-DAG: %[[AND_INV_2:.+]] = aig.and_inv not %[[AND_INV_0]], not %[[AND_INV_0]]
  // CHECK-DAG: %[[AND_INV_3:.+]] = aig.and_inv not %[[AND_INV_1]], not %[[AND_INV_1]]

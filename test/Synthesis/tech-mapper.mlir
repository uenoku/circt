// RUN: circt-opt --pass-pipeline='builtin.module(synthesis-tech-mapper{strategy=area test=true})' %s | FileCheck %s --check-prefixes CHECK,AREA
// RUN: circt-opt --pass-pipeline='builtin.module(synthesis-tech-mapper{strategy=timing test=true})' %s | FileCheck %s --check-prefixes CHECK,TIMING

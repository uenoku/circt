// RUN: circt-opt --pass-pipeline='builtin.module(hw.module(synthesis-generic-lut-mapper))' %s | FileCheck %s --check-prefixes CHECK,LUT
// RUN: circt-opt --pass-pipeline='builtin.module(hw.module(synthesis-generic-lut-mapper{max-lut-size=2}))' %s | FileCheck %s --check-prefixes CHECK,LUT2

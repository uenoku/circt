...
enum Until { UntilAIG, UntilAIGOpt, UntilAIGLowering, UntilEnd };

static auto runUntilValues = llvm::cl::values(
    clEnumValN(UntilAIG, "aig", "Conversion of modules to AIG"),
    clEnumValN(UntilAIGOpt, "aig-opt", "AIG optimizations"),
    clEnumValN(UntilAIGLowering, "aig-lowering",
               "AIG lowering to lut-mappable forms"),
    clEnumValN(UntilEnd, "all", "After AIG to Comb lowering"));
...
static void populateSynthesisPipeline(PassManager &pm) {
  ...
  mpm.addPass(createConvertCombToAIG());
  mpm.addPass(createCSEPass());
  if (untilReached(UntilAIG))
    return;
...
int main(int argc, char **argv) {
  ...
  cl::ParseCommandLineOptions(argc, argv,
                              "circt-synth - Logic synthesis tool\n\n");

#!/usr/bin/env python3
"""
Python script to parse "top_paths" from JSON and convert to type-safe dataclasses.

This script parses JSON output from CIRCT's longest path analysis and converts
it into type-safe Python dataclasses for easier manipulation and analysis.
"""
...
class DataflowPath:
    ...
    def to_flamegraph_format(self, include_history: bool = True) -> str:
        """
        Convert this path to FlameGraph format using instance hierarchy as stack.

        FlameGraph format: "stack_trace delay"
        Where stack_trace represents the instance path hierarchy through the design.

        Args:
            include_history: If True, include debug history points in the stack

        Returns:
            String in FlameGraph format
        """
        # Build the complete instance path from fan-in to fan-out
        stack_elements = []

        # Start with root module
        stack_elements.append(self.root)

        # Use the deepest instance path (usually fan-out has the most complete path)
        instance_path = self.fan_out.instance_path
        if not instance_path and self.path.fan_in.instance_path:
            instance_path = self.path.fan_in.instance_path

        # Add each level of the instance hierarchy
        for elem in instance_path:
            stack_elements.append(f"{elem.module_name}::{elem.instance_name}")

        # Add the signal name as the leaf
        signal_name = self.fan_out.name
        if self.fan_out.bit_pos > 0:
            signal_name += f"[{self.fan_out.bit_pos}]"
        stack_elements.append(signal_name)

        # Join with semicolons and add delay as the "time"
        stack_trace = ";".join(stack_elements)
        return f"{stack_trace} {self.path.delay}"

    def to_flamegraph_format_detailed(self, include_history: bool = True) -> str:
        """

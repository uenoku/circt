...
struct MatchResult {
  ...
  DelayType getDelay(unsigned inputIndex, unsigned outputIndex,
    ...
    return getDelays()[outputIndex * numInputs + inputIndex];
  }
...
struct CutRewritePattern {
  ...
  /// \return MatchResult if the cut matches, std::nullopt otherwise.
  virtual std::optional<MatchResult> match(const Cut &cut,
                                           CutEnumerator &enumerator) const = 0;

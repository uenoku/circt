...
public:
  /// Required time propagated from primary outputs during area recovery.
  DelayType requiredTime = std::numeric_limits<DelayType>::max();

  /// Arrival time of the currently selected cut under the current reselection.
  DelayType bestArrivalTime = 0;

  /// Area-flow score of the currently selected cut.
  double areaFlow = 0.0;

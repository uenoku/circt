...
private:
  ...
  void buildSuffixTree(TimingNode *endpoint,
                       llvm::DenseMap<TimingNode *, SuffixTreeEntry> &sfxt);

  /// Extract K worst paths for an endpoint using PFXT.
  void extractKPaths(TimingNode *endpoint, size_t k,
                     const llvm::DenseMap<TimingNode *, SuffixTreeEntry> &sfxt,
                     const llvm::DenseSet<TimingNode *> &validStartPoints,
                     SmallVectorImpl<TimingPath> &results);

  /// Reconstruct full path using suffix tree.
  void reconstructPathViaSFXT(
      TimingNode *start, TimingNode *end,
      const llvm::DenseMap<TimingNode *, SuffixTreeEntry> &sfxt,
      TimingPath &path);

  /// Check if a path goes through any of the specified nodes.
  bool goesThrough(const TimingPath &path,

...
void MiniSATSolver::backtrack(int level) {
  ...
  // - For level = 0: clear levels >0 (keep level-0 propagations/units)
  int clearFrom;
  if (level == 0) {
    clearFrom = static_cast<int>(trailLimits.size()) > 0 ? trailLimits[0]
...
void MiniSATSolver::clearTransientState() {
  backtrack(0);
  rootLevel = 0;
  propagHead = 0;
  for (auto &var : vars)
...
void MiniSATSolver::clearLearnts() {
  if (learntClauses.empty() && learntBinaryClauses.empty())
    return;
  learntClauses.clear();
...
void MiniSATSolver::analyze(const Conflict &confl,
  ...
  auto isValidLit = [this](int lit) {
    int v = litVar(lit);
    return v >= 0 && v < static_cast<int>(vars.size());
  };
  ...
  if (invalidState) {
    for (size_t i = 1; i < outLearnt.size(); ++i)
      if (isValidLit(outLearnt[i]))
        vars[litVar(outLearnt[i])].seen = 0;
    outLearnt.clear();
    outBackLevel = rootLevel;
    return;
  }

  // A conflict without any literal at the current decision level cannot
  // produce a valid 1-UIP asserting clause for this level.
  if (pathCount == 0) {
    for (size_t i = 1; i < outLearnt.size(); ++i)
      vars[litVar(outLearnt[i])].seen = 0;
    outLearnt.clear();
  ...
  while (pathCount > 0) {
    while (trailIdx >= 0 &&
           (!isValidLit(trail[trailIdx]) || !vars[litVar(trail[trailIdx])].seen))
      trailIdx--;
    if (trailIdx < 0) {
      for (size_t i = 1; i < outLearnt.size(); ++i)
        if (isValidLit(outLearnt[i]))
...
MiniSATSolver::Result MiniSATSolver::solveImpl(int64_t confLimit) {
  llvm::SmallVector<int, 8> assumptions(assumptionBuf.begin(),
                                        assumptionBuf.end());
  bool hadAssumptions = !assumptions.empty();
  auto finalizeTransient = [&]() {
    backtrack(0);
    rootLevel = 0;
    propagHead = static_cast<int>(trail.size());
    for (auto &var : vars)

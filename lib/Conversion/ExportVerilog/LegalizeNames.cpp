...
GlobalNameResolver::GlobalNameResolver(mlir::ModuleOp topLevel,
  ...
  for (auto &op : *topLevel.getBody()) {
    ...
    if (isa<HWModuleExternOp>(op) || isa<HWModuleGeneratedOp>(op)) {
      ...
      // For macro modules, set the verilogName to the macro's name with
      // backtick.
      auto *macroDecl =
      ...
      StringRef macroName =
          verilogName ? *verilogName : macroDeclOp.getSymName();
      // Set the verilogName attribute with backtick prefix.
      std::string nameWithBacktick = ("`" + macroName).str();
      macroModule->setAttr("hw.verilogName",
                           StringAttr::get(op.getContext(), "`" + macroName));

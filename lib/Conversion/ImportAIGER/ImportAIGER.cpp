...
private:
  ...
  InFlightDiagnostic emitWarning(const Twine &message) {
    return mlir::emitWarning(lexer.translateLocation(lexer.getCurrentLoc()),
                             message);
  }
...
ParseResult AIGERParser::parseAndGatesBinary() {
  ...
  unsigned currentLHS = 2 * (numInputs + numLatches + 1);
  ...
  for (unsigned i = 0; i < numAnds; ++i) {
    ...
    int64_t rhs0 = lhs - delta0;
    int64_t rhs1 = rhs0 - delta1;
    ...
    // Validate ordering constraints (deltas can be 0 in some cases)
    if (rhs0 >= lhs || rhs1 > rhs0)
      return emitError("invalid binary AND gate: ordering constraint violated");
...
ParseResult AIGERParser::createModule() {
  ...
  for (auto [i, latchDef] : llvm::enumerate(latchDefs)) {
    auto [currentState, nextState] = latchDef;

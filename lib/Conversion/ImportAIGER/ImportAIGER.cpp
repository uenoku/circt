//===- ImportAIGER.cpp - AIGER file import --------------------*- C++ -*-===//
//
...
enum class AIGERTokenKind {
  ...
  // Keywords
  AAG, // ASCII format identifier
  AIG, // Binary format identifier
...
private:
  ...
  // Value mappings - unified using backedges
  DenseMap<unsigned, StringRef> symbolTable;

  // Parsed data storage
  SmallVector<unsigned> inputLiterals;
  SmallVector<std::pair<unsigned, unsigned>> latchDefs; // current, next
  SmallVector<unsigned> outputLiterals;
  SmallVector<std::tuple<unsigned, unsigned, unsigned>>
  ...
  /// Create the top-level HW module
  ParseResult createModule();

  /// Emit error at current location
  InFlightDiagnostic emitError(const Twine &message) {
    return mlir::emitError(lexer.translateLocation(lexer.getCurrentLoc()),
                           message);
  }
...
AIGERToken AIGERLexer::lexNumber() {
  ...
  while (curPtr < curBuffer.end() && std::isdigit(*curPtr))
    ++curPtr;
...
AIGERToken AIGERLexer::lexIdentifier() {
  const char *start = curPtr;
  while (curPtr < curBuffer.end() && (std::isalnum(*curPtr) || *curPtr == '_'))
    ++curPtr;
  ...
  // Check for keywords
  if (spelling == "aag")
    kind = AIGERTokenKind::AAG;
  else if (spelling == "aig")
    kind = AIGERTokenKind::AIG;
...
AIGERToken AIGERLexer::nextToken() {
  ...
  default:
    ...
    // Not officially part of the AIGER format, but we support it for FileCheck
    // tests.
    if (c == '/') {
      if (*curPtr == '/') {
        ++curPtr;
...
ParseResult AIGERParser::parseNumber(unsigned &result) {
  ...
  if (token.kind != AIGERTokenKind::Number) {
    return emitError("expected number");
  }
...
ParseResult AIGERParser::parseHeader() {
  ...
  AIGERToken formatToken = lexer.nextToken();
  if (formatToken.kind == AIGERTokenKind::AAG) {
    isBinaryFormat = false;
    LLVM_DEBUG(llvm::dbgs() << "Format: aag (ASCII)\n");
  } else if (formatToken.kind == AIGERTokenKind::AIG) {
    isBinaryFormat = true;
    LLVM_DEBUG(llvm::dbgs() << "Format: aig (binary)\n");
  } else {
    return emitError("expected 'aag' or 'aig' format identifier");
  }
...
ParseResult AIGERParser::parseInputs() {
  // In AIGER format, inputs are implicit (literals 2, 4, 6, ...)
  // Both ASCII and binary formats use implicit input numbering
  for (unsigned i = 0; i < numInputs; ++i) {
    unsigned literal;
    if (parseNumber(literal))
      return emitError("failed to parse input literal");
...
ParseResult AIGERParser::parseLatches() {
  ...
  for (unsigned i = 0; i < numLatches; ++i) {
    ...
    if (parseNumber(currentState) || parseNumber(nextState))
      return emitError("failed to parse latch definition");
    ...
    if (currentState % 2 != 0 || currentState == 0)
      return emitError("invalid latch current state literal");

    latchDefs.push_back({currentState, nextState});
  }
...
ParseResult AIGERParser::parseOutputs() {
  ...
  for (unsigned i = 0; i < numOutputs; ++i) {
    unsigned literal;
    if (parseNumber(literal))
      return emitError("failed to parse output literal");
...
ParseResult AIGERParser::parseAndGates() {
  ...
  for (unsigned i = 0; i < numAnds; ++i) {
    ...
    if (parseNumber(lhs) || parseNumber(rhs0) || parseNumber(rhs1))
      return emitError("failed to parse AND gate definition");
    LLVM_DEBUG(llvm::dbgs() << "AND Gate " << i << ": " << lhs << " = " << rhs0
                            << " & " << rhs1 << "\n");

    // Validate LHS (should be even and positive)
    if (lhs % 2 != 0 || lhs == 0)
      return emitError("invalid AND gate LHS literal");

    // Validate literal bounds
    if (lhs / 2 > maxVarIndex || rhs0 / 2 > maxVarIndex ||
        rhs1 / 2 > maxVarIndex)
      return emitError("AND gate literal exceeds maximum variable index");

    // In ASCII format, ensure proper ordering
    if (isBinaryFormat) {
      if ((lhs <= rhs0 || lhs <= rhs1))
        return emitError("AND gate LHS must be greater than RHS literals");

      // AIGER format requires rhs0 >= rhs1
      if (rhs0 < rhs1)
        return emitError("AND gate requires rhs0 >= rhs1");
    }

    andGateDefs.push_back({lhs, rhs0, rhs1});
  }
...
ParseResult AIGERParser::parseSymbolTable() {
  ...
  while (!lexer.isAtEOF()) {
    ...
    if (token.spelling != "i" && token.spelling != "l" && token.spelling != "o")
      break;

    // Consume the symbol type
    lexer.nextToken();

    // Parse position
    unsigned position;
    if (parseNumber(position))
      return emitError("failed to parse symbol position");

    // Parse symbol name (rest of line)
    // For now, we'll skip symbol table parsing
    // TODO: Implement full symbol table support

    break; // Skip symbol table for now
  }
...
ParseResult AIGERParser::createModule() {
  ...
  for (unsigned i = 0; i < numInputs; ++i) {
    hw::PortInfo port;
    port.name = builder.getStringAttr("input_" + std::to_string(i));
  ...
  for (unsigned i = 0; i < numOutputs; ++i) {
    hw::PortInfo port;
    port.name = builder.getStringAttr("output_" + std::to_string(i));
    port.type = builder.getI1Type();
  ...
  auto hwModule = builder.create<hw::HWModuleOp>(
      loc, builder.getStringAttr(moduleName), ports);
  ...
  for (auto [currentState, nextState] : latchDefs) {
    backedges[currentState] = bb.get(builder.getI1Type());
  }
  for (auto [lhs, rhs0, rhs1] : andGateDefs) {
  ...
  for (auto [currentState, nextState] : latchDefs) {
    ...
    Backedge nextBackedge = bb.get(builder.getI1Type());

    // Create the register with the backedge as input
    Value regValue =
        builder.create<seq::CompRegOp>(loc, (Value)nextBackedge, clockValue);
  ...
  for (auto [lhs, rhs0, rhs1] : andGateDefs) {
    ...
    Value rhs0Value = getLiteralValue(rhs0 & ~1u, backedges, loc);
    Value rhs1Value = getLiteralValue(rhs1 & ~1u, backedges, loc);

    if (!rhs0Value || !rhs1Value)
      return emitError("failed to get operand values for AND gate");
    ...
    Value andResult = builder.create<aig::AndInverterOp>(
        loc, builder.getI1Type(), ValueRange{rhs0Value, rhs1Value}, inverts);
  ...
  // values
  for (auto [currentState, nextState] : latchDefs) {
    Value nextValue = getLiteralValue(nextState, backedges, loc);
    if (!nextValue)
      return emitError("undefined literal in latch next state");
    ...
    if (auto regOp = currentValue.getDefiningOp<seq::CompRegOp>()) {
      ...
      return emitError("failed to find register for latch");
    }
  }

  // Create output values
  SmallVector<Value> outputValues;
  for (unsigned literal : outputLiterals) {
    Value outputValue = getLiteralValue(literal, backedges, loc);
    if (!outputValue)
      return emitError("undefined literal in output");
...
LogicalResult circt::importAIGER(llvm::SourceMgr &sourceMgr,
...
void circt::registerFromAIGERTranslation() {

...
private:
  ...
  /// Parse an optional number (doesn't fail if no number present)
...
ParseResult AIGERParser::createModule() {
  ...
  for (auto [lhs, rhs0, rhs1] : andGateDefs) {
    ...
    auto rhs0Value = getLiteralValue(rhs0 & ~1u, backedges, loc);
    auto rhs1Value = getLiteralValue(rhs1 & ~1u, backedges, loc);
  ...
  for (auto literal : outputLiterals) {

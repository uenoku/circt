//===- ExportRTLIL.cpp - Export core dialects to RTLIL --------------------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines an exporter for Yosys RTLIL.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "export-rtlil"
...
struct ExportRTLILModule
  ...
  LogicalResult visitSeq(seq::CompRegOp op);
...
LogicalResult ExportRTLILModule::visitTypeOp(ArrayGetOp op) {
  ...
  auto elemSize = hw::getBitWidth(
      hw::type_cast<hw::ArrayType>(op.getInput().getType()).getElementType());
  index.value().extend_u0(bitWidth);
  auto sig = rtlilModule->Mul(NEW_ID, index.value(),
                              getConstant(APInt(bitWidth, elemSize)), false);

  (void)rtlilModule->addShr(NEW_ID, input.value(), sig, result.value());
  return success();
}
...
LogicalResult ExportRTLILModule::visitSeq(seq::CompRegOp op) {
  auto result = getValue(op.getResult());
  auto clock = getValue(op.getClk());
  auto next = getValue(op.getInput());
  ...
  if (op.getReset()) {
    ...
    if (constOp) {
      rtlilModule->addSdff(getNewName(op.getName() ? *op.getName() : ""),
                           clock.value(), reset.value(), next.value(),
                           result.value(), getConstant(constOp.getValueAttr()));
      return success();
    }
    return op.emitError() << "lowering for non-constant reset value is "
                             "currently not implemented";
  ...
  rtlilModule->addDff(getNewName(op.getName() ? *op.getName() : ""),
                      clock.value(), next.value(), result.value());
  return success();
}

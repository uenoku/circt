...
static Value createMajority(OpBuilder &rewriter, Location loc, Value a, Value b,
                            Value carry, bool useMIG) {
  if (useMIG) {
...
struct AndInverterVariadicOpConversion : OpRewritePattern<aig::AndInverterOp> {
...
struct CombAddOpConversion : OpConversionPattern<AddOp> {
  ...
  void lowerRippleCarryAdder(comb::AddOp op, ValueRange inputs,
    ...
    for (int64_t i = 0; i < width; ++i) {
      ...
      carry = createMajority(rewriter, op.getLoc(), aBits[i], bBits[i], carry,
                             useMIG);
    }
  ...
  void lowerKoggeStonePrefixTree(comb::AddOp op, ValueRange inputs,
    ...
    for (int64_t stride = 1; stride < width; stride *= 2) {
      for (int64_t i = stride; i < width; ++i) {
        ...
        if (useMIG) {
          // Group generate using majority: g_i | (p_i & g_j) = maj(g_i, p_i,
          // g_j) This works because: maj(a,b,c) = (a&b) | (a&c) | (b&c) When
          // a=g_i=1, result is 1. When a=g_i=0, result is b&c = p_i&g_j
          SmallVector<Value, 3> inputs = {gPrefix[i], pPrefix[i], gPrefix[j]};
          SmallVector<bool, 3> inverts = {false, false, false};
          gPrefixNew[i] = synth::mig::MajorityInverterOp::create(
              rewriter, op.getLoc(), inputs, inverts);
        } else {
          // Standard implementation: g_i OR (p_i AND g_j)
          Value andPG = comb::AndOp::create(rewriter, op.getLoc(), pPrefix[i],
                                            gPrefix[j]);
          gPrefixNew[i] =
...
static void
populateCombToSynthConversionPatterns(RewritePatternSet &patterns,
  ...
  if (useMIG) {
    patterns.add<CombOrToMIGConversion, CombLowerVariadicOp<OrOp>,
                 AndInverterToMIGConversion, AndInverterVariadicOpConversion,
...
void ConvertCombToAIGPass::runOnOperation() {
  ...
  populateCombToSynthConversionPatterns(patterns, maxEmulationUnknownBits,
                                        loweringTarget ==

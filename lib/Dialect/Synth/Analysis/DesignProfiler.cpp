    ...
    for (auto &[clockPair, stats] : regToRegStats) {
      ...
      if (srcClockTree && dstClockTree) {
        if (haveCommonClockGate(srcClockTree, dstClockTree)) {
          os << "  ⚠️  Common clock gate detected between source and "
                "destination clocks\n";
        }
      }
...
ClockDomain
DesignProfilerPass::getClockForTimingNode(const timing::TimingNode *node) {
  if (!node)
    return {};
...
PathCategory
DesignProfilerPass::categorizeTwoStage(const timing::TimingPath &path) {

...
public:
  FRAIGSolver(hw::HWModuleOp module, unsigned numPatterns,
              unsigned conflictLimit, bool enableFeedback, unsigned seed)
      : module(module), numPatterns(numPatterns), conflictLimit(conflictLimit),
        enableFeedback(enableFeedback), seed(seed) {}
...
void FRAIGSolver::verifyCandidates() {
  ...
  // Use ONE persistent solver for entire verification phase.
  // ABC-style bookmark/rollback enables learned clause sharing across queries.
  ...
  for (size_t ecIdx = 0; ecIdx < equivClasses.size(); ++ecIdx) {
    ...
    // Bookmark: save state before adding cone-specific CNF
    solver.bookmark();
    ...
    // Rollback: remove cone CNF, keep learned clauses for next class!
    solver.rollback();
  }
...
struct FunctionalReductionPass
  ...
  void runOnOperation() override {
    ...
    // Create and run FRAIG solver with seed
    FRAIGSolver fraig(module, numRandomPatterns, satConflictLimit,
                      enableCEXFeedback, seed);

...
// optimization using a Z3-backed incremental SAT solver. It identifies and
...
private:
  ...
  unsigned computeDepth(Value v, llvm::DenseSet<Value> &visiting);
  ...
  int getOrCreateLocalVar(Value v, llvm::DenseMap<Value, int> &localVarMap,
                          int &localNextVar);
  void addLocalStructuralConstraints(IncrementalSATSolver &s, Value v,
  ...
  static void addMajority3Clauses(IncrementalSATSolver &s, int outVar, int a,
                                  int b, int c);

  // CEX feedback: refine equivalence classes using counterexample from SAT
  void refineByCEX(IncrementalSATSolver &solver,
...
unsigned FRAIGSolver::computeDepth(Value v) {
  ...
  return computeDepth(v, visiting);
}
...
unsigned FRAIGSolver::computeDepth(Value v,
  ...
  if (!visiting.insert(v).second) {
    LLVM_DEBUG(llvm::dbgs() << "FRAIG: Cycle detected in depth computation for "
                            << v << "\n");
    return 0;
  }
  ...
  for (Value operand : op->getOperands()) {
    if (operand.getType().isInteger(1))
      maxInputDepth = std::max(maxInputDepth, computeDepth(operand, visiting));
  }
...
//
// For each equivalence class, we create a fresh incremental SAT solver and encode
...
int FRAIGSolver::getOrCreateLocalVar(
...
void FRAIGSolver::addMajority3Clauses(IncrementalSATSolver &s, int outVar, int a,
                                      int b, int c) {
...
void FRAIGSolver::addLocalStructuralConstraints(
    IncrementalSATSolver &s, Value v, llvm::DenseMap<Value, int> &localVarMap,
...
void FRAIGSolver::refineByCEX(
    IncrementalSATSolver &solver, llvm::DenseMap<Value, int> &varMap,
    const llvm::DenseSet<Value> &visited) {
...
void FRAIGSolver::verifyCandidates() {
  ...
  // Global solver:
  // - Pre-allocate variables for all values
  // - Encode each value exactly once
  // - Learn permanent equivalence clauses across equivalence classes
  auto solver = createZ3SATSolver();
  if (!solver) {
    module.emitError()
        << "FunctionalReduction requires Z3 support, but createZ3SATSolver() "
           "returned null";
    return;
  }
  ...
  // Add structural clauses for all i1 values once.
  llvm::DenseSet<Value> constrainedValues;
  int localNextVar = maxVar;
  for (auto value : allValues)
    addLocalStructuralConstraints(*solver, value, globalVarMap, localNextVar,
                                 constrainedValues);
  ...
  for (size_t ecIdx = 0; ecIdx < equivClasses.size(); ++ecIdx) {
    ...
    // Already constrained for all values above.
    ...
    for (size_t mIdx = 0; mIdx < ec.members.size(); ++mIdx) {
      ...
      if (result1 == IncrementalSATSolver::kSAT) {
        stats.numDisprovedEquiv++;
        LLVM_DEBUG(llvm::dbgs() << "FRAIG: Disproved " << member << "\n");
        refineByCEX(*solver, globalVarMap, constrainedValues);
        continue;
      }

      if (result1 != IncrementalSATSolver::kUNSAT) {
        stats.numUnknown++;
      ...
      if (result2 == IncrementalSATSolver::kSAT) {
        stats.numDisprovedEquiv++;
        LLVM_DEBUG(llvm::dbgs() << "FRAIG: Disproved " << member << "\n");
        refineByCEX(*solver, globalVarMap, constrainedValues);
        continue;
      ...
      if (result2 != IncrementalSATSolver::kUNSAT) {
        stats.numUnknown++;
        LLVM_DEBUG(llvm::dbgs()
                   << "FRAIG: Unknown (conflict limit) for " << member << "\n");
        continue;
...
struct FunctionalReductionPass
  ...
  void runOnOperation() override {
    ...
    if (backend == "auto" || backend == "z3") {
      FRAIGSolver fraig(module, numRandomPatterns,

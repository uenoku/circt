...
template <typename PrimarySATSolverT, typename OracleSATSolverT>
class CrossCheckingSATSolver
    : public IncrementalSATSolverBase<
          CrossCheckingSATSolver<PrimarySATSolverT, OracleSATSolverT>> {
public:
  using Result = typename IncrementalSATSolverBase<
      CrossCheckingSATSolver<PrimarySATSolverT, OracleSATSolverT>>::Result;
  ...
  Result solve(int64_t confLimit = -1) {
    Result primaryResult = static_cast<Result>(primary.solve(confLimit));
    Result oracleResult = static_cast<Result>(oracle.solve(confLimit));

    if (static_cast<int>(primaryResult) != static_cast<int>(oracleResult)) {
      emitMismatch("result", primaryResult, oracleResult);
      assumptions.clear();
      lastSolveWasSat = false;
      return primaryResult;
    }

    lastSolveWasSat = (primaryResult == Result::kSAT);
    assumptions.clear();
    return primaryResult;
  }

  int val(int v) const {
    int primaryVal = primary.val(v);
    if (!lastSolveWasSat)
      return primaryVal;

    int oracleVal = oracle.val(v);
    if (primaryVal != oracleVal)
      emitModelMismatch(v, primaryVal, oracleVal);
    return primaryVal;
  }
  ...
  void bookmark() {
    primary.bookmark();
    oracle.bookmark();
    bookmarkClauseCount = clauses.size();
  }
  ...
  void rollback() {
    ...
    assumptions.clear();
    lastSolveWasSat = false;
    clauses.resize(bookmarkClauseCount);
  }

  bool hasBookmark() const { return primary.hasBookmark(); }

  void reserveVars(int maxVar) {
    primary.reserveVars(maxVar);
    oracle.reserveVars(maxVar);
  }

  void dumpDIMACS(llvm::raw_ostream &os,
                  llvm::ArrayRef<int> extraAssumptions = {}) const {
    int maxVar = 0;
    for (const auto &clause : clauses)
      for (int lit : clause)
        maxVar = std::max(maxVar, std::abs(lit));
    for (int lit : extraAssumptions)
      maxVar = std::max(maxVar, std::abs(lit));

    os << "p cnf " << maxVar << " " << (clauses.size() + extraAssumptions.size())
       << "\n";
    for (const auto &clause : clauses) {
      for (int lit : clause)
        os << lit << " ";
...
private:
  ...
  void emitMismatch(const char *kind, Result primaryResult,
                    Result oracleResult) const {
    llvm::errs() << "FunctionReduction SAT " << kind
                 << " mismatch: primary=" << resultToString(primaryResult)
                 << " oracle=" << resultToString(oracleResult) << "\n";
    dumpReproducer();
  }

  void emitModelMismatch(int var, int primaryVal, int oracleVal) const {
    llvm::errs() << "FunctionReduction SAT model mismatch on var " << var
                 << ": primary=" << primaryVal << " oracle=" << oracleVal
                 << "\n";
    dumpReproducer();
  }

  void dumpReproducer() const {
    static int mismatchCount = 0;
    std::string filename = "/tmp/function_reduction_sat_mismatch_" +
                           std::to_string(++mismatchCount) + ".cnf";
    std::error_code ec;
    llvm::raw_fd_ostream os(filename, ec, llvm::sys::fs::OF_Text);
    if (ec)
      return;
    dumpDIMACS(os, assumptions);
  }

  PrimarySATSolverT primary;
  OracleSATSolverT oracle;
  llvm::SmallVector<llvm::SmallVector<int, 4>, 0> clauses;
  llvm::SmallVector<int, 4> currentClause;
  llvm::SmallVector<int, 4> assumptions;
  size_t bookmarkClauseCount = 0;
  bool lastSolveWasSat = false;
};
...
public:
  ...
  [[nodiscard]] Result solve(int64_t confLimit = -1) {
    ...
    Result cadicalResult = (res == CaDiCaL::SATISFIABLE   ? kSAT
  ...
  [[nodiscard]] int val(int v) const { return solver->val(v); }
...
struct FunctionalReductionPass
  ...
  void runOnOperation() override {
    ...
    auto backend = llvm::StringRef(satBackend).lower();

    LLVM_DEBUG(llvm::dbgs() << "Running FunctionalReduction (FRAIG) pass on "
                            << module.getName() << " with backend "
                            << backend << "\n");

    auto updateStats = [&](const auto &stats) {
    ...
    if (backend == "mini") {
      ...
      updateStats(fraig.run());
      return;
    }

#ifdef CIRCT_CADICAL_ENABLED
    if (backend == "cadical" || backend == "auto") {
    ...
    if (backend == "cross-check") {
      using CheckingSolver =
          CrossCheckingSATSolver<MiniSATSolver, CadicalSATSolver>;
      FRAIGSolver<CheckingSolver> fraig(module, numRandomPatterns,
                                        satConflictLimit, seed);
      updateStats(fraig.run());
      return;
    }
#else
    if (backend == "cadical" || backend == "cross-check") {
      module.emitError()
          << "Requested sat-backend '" << satBackend
          << "' but this build does not include CaDiCaL support";
      signalPassFailure();
      return;
    }
#endif
    ...
    if (backend != "auto") {
      module.emitError() << "Unknown sat-backend '" << satBackend
                         << "' (expected auto, mini, cadical, or cross-check)";
      signalPassFailure();
      return;
    }

    // Fall back to built-in MiniSAT solver when CaDiCaL is unavailable.
    FRAIGSolver<MiniSATSolver> fraig(module, numRandomPatterns,
                                     satConflictLimit, seed);
    updateStats(fraig.run());
  }

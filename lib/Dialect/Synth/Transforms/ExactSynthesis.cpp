...
static GreedyPatternInverterKind
toGreedyPatternInverterKind(CutRewriteInverterKind kind) {
  switch (kind) {
  case CutRewriteInverterKind::mig:
    return GreedyPatternInverterKind::Mig;
  case CutRewriteInverterKind::aig:
    return GreedyPatternInverterKind::Aig;
  case CutRewriteInverterKind::dig:
    return GreedyPatternInverterKind::Dig;
  }
...
struct LoadedExactNetworkEntry : public LoadedCutRewriteEntry {
  ...
  std::optional<GreedyPatternBlock> getGreedyPattern() const override {
    auto module = cast<hw::HWModuleOp>(moduleOp);
    auto output = dyn_cast<hw::OutputOp>(module.getBodyBlock()->getTerminator());
    if (!output || output.getNumOperands() != 1)
      return std::nullopt;
    return GreedyPatternBlock{module.getBodyBlock(), output.getOperand(0),
                              toGreedyPatternInverterKind(inverterKind)};
  }

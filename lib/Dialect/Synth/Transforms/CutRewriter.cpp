...
struct MergedTruthTableBuilder {
  ...
  void getInputMapping(const Cut *cut,
    ...
    for (uint32_t idx : cut->inputs) {
      // Both input lists are sorted, so one forward walk is enough and avoids
      // per-input binary search or hashing overhead on these tiny cuts.
...
const CutSet *CutEnumerator::getCutSet(uint32_t index) {
  ...
  if (it == cutSets.end()) {
    ...
    CutSet *cutSet = cutSetAllocator.create();
    Cut *trivialCut = cutAllocator.create(getAsTrivialCut(index));

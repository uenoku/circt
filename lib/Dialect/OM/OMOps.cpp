...
namespace {
struct FoldFrozenBasePathCreate : OpRewritePattern<FrozenBasePathCreateOp> {
  using OpRewritePattern::OpRewritePattern;

  LogicalResult matchAndRewrite(FrozenBasePathCreateOp op,
                                PatternRewriter &rewriter) const override {
    Attribute attr;
    if (!matchPattern(op.getBasePath(), m_Constant(&attr)))
      return failure();
    auto basePathAttr = dyn_cast<FrozenBasePathAttr>(attr);
    if (!basePathAttr)
      return failure();

    rewriter.replaceOpWithNewOp<ConstantOp>(
        op,
        FrozenBasePathAttr::get(
            op.getContext(), appendPath(basePathAttr.getPath(), op.getPath())));
    return success();
  }
};
} // namespace

void circt::om::FrozenBasePathCreateOp::getCanonicalizationPatterns(
    RewritePatternSet &results, MLIRContext *context) {
  results.add<FoldFrozenBasePathCreate>(context);
}
...
namespace {
struct FoldFrozenPathCreate : OpRewritePattern<FrozenPathCreateOp> {
  using OpRewritePattern::OpRewritePattern;

  LogicalResult matchAndRewrite(FrozenPathCreateOp op,
                                PatternRewriter &rewriter) const override {
    Attribute attr;
    if (!matchPattern(op.getBasePath(), m_Constant(&attr)))
      return failure();
    ...
    rewriter.replaceOpWithNewOp<ConstantOp>(
        op, FrozenPathAttr::get(
                op.getContext(), op.getTargetKindAttr(),
                appendPath(basePathAttr.getPath(), op.getPath()),
                op.getModuleAttr(), op.getRefAttr(), op.getFieldAttr()));
    return success();
  }
};
} // namespace

void circt::om::FrozenPathCreateOp::getCanonicalizationPatterns(
    RewritePatternSet &results, MLIRContext *context) {
  results.add<FoldFrozenPathCreate>(context);
}
...
namespace {
struct FoldFrozenEmptyPath : OpRewritePattern<FrozenEmptyPathOp> {
  using OpRewritePattern::OpRewritePattern;

  LogicalResult matchAndRewrite(FrozenEmptyPathOp op,
                                PatternRewriter &rewriter) const override {
    rewriter.replaceOpWithNewOp<ConstantOp>(
        op, FrozenEmptyPathAttr::get(op.getContext()));
    return success();
  }
};
} // namespace

void circt::om::FrozenEmptyPathOp::getCanonicalizationPatterns(
    RewritePatternSet &results, MLIRContext *context) {
  results.add<FoldFrozenEmptyPath>(context);
}

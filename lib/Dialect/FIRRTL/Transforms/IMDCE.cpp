...
struct IMDCEPass : public IMDCEBase<IMDCEPass> {
  ...
  /// Return true if the value is known live
  bool isKnownLive(Value value) const {
    assert(value && "null should not be used");
    return liveSet.count(value);
  }

  /// Return true if the value is assumed dead.
  bool isAssumedDead(Value value) const { return !isKnownLive(value); }
...
void IMDCEPass::markWireOrReg(Operation *op) {
  // If the wire/reg has a non-ground type, mark live for now.
  auto resultValue = op->getResult(0);

  if (!isDeletableWireOrRegOrNode(op))
    markLive(resultValue);
}
...
void IMDCEPass::markInstanceOp(InstanceOp instance) {
  ...
  if (!isa<FModuleOp>(op)) {
    ...
    for (size_t resultNo = 0, e = instance.getNumResults(); resultNo != e;
      ...
      // Otherwise this is an inuput from it or an inout, mark it as live.
      markLive(portVal);
    }
    return;
  }
  ...
  for (size_t resultNo = 0, e = instance.getNumResults(); resultNo != e;
    ...
    // Mark don't touch results as live.
    if (hasDontTouch(modulePortVal)) {
      markLive(modulePortVal);
      markLive(instancePortVal);
    }
...
void IMDCEPass::runOnOperation() {
  ...
  // Mark the ports of public modules as live.
...
void IMDCEPass::visitValue(Value value) {
  assert(isKnownLive(value) && "only live values reach here");
  ...
  if (auto instance = value.getDefiningOp<InstanceOp>()) {
    ...
    return markLive(modulePortVal);
  }

  // If op is defined by an operation, mark its operands as live.
...
void IMDCEPass::visitConnect(FConnectLike connect) {
  // If the dest is live, mark the source value as live.
  if (isKnownLive(connect.dest()))
    markLive(connect.src());
}

void IMDCEPass::visitSubelement(Operation *op) {
  if (isKnownLive(op->getOperand(0)))
    markLive(op->getResult(0));
}
...
void IMDCEPass::rewriteModuleSignature(FModuleOp module) {
  ...
  for (auto index : llvm::seq(0u, numOldPorts)) {
    ...
    assert((!hasDontTouch(argument) || isKnownLive(argument)) &&
           "If the port has don't touch, it should be known live");

    // If the port has dontTouch, skip.
    if (hasDontTouch(argument))
      continue;

    // If the port is known live, then we can't delete it except for write-only
    // output ports.
    if (isKnownLive(argument)) {
      bool deadOutputPortAtAnyInstantiation =
          module.getPortDirection(index) == Direction::Out &&
          llvm::all_of(instanceGraphNode->uses(), [&](auto *use) {
            auto instance = cast<InstanceOp>(*use->getInstance());
            return isAssumedDead(instance.getResult(index));
          });

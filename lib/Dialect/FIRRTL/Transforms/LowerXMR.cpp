...
class LowerXMRPass : public circt::firrtl::impl::LowerXMRBase<LowerXMRPass> {
  ...
  /// Collects ref port information for later header generation in
  /// handlePublicModuleRefPorts.
  ///
  /// Example output (generated by handlePublicModuleRefPorts):
    ...
    for (size_t portNum = 0; portNum < numPorts; ++portNum) {
      ...
      if (parentModule.isPublic()) {
        instanceChoiceRefPorts[parentModule].push_back(
            {macroNameAttr, portNum, instanceMacro, optionName, targetChoices,
             defaultTarget, inst});
      }
    }
  ...
  /// Generate macro definition for an instance choice ref port with ifdef
  /// guards.
  LogicalResult
  ...
  LogicalResult handlePublicModuleRefPorts(FModuleOp module) {
    ...
    emit::FileOp::create(fileBuilder, fileName, [&] {
      ...
      // Generate macro definitions for instance choice ref ports.

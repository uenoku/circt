...
static llvm::LogicalResult convertDivModUByPowerOfTwo(PatternRewriter &rewriter,
  ...
  if (isDiv) {
    ...
    // If shifting by the full width or more, result is 0.
    if (bitPosition >= width) {
      rewriter.replaceOpWithNewOp<hw::ConstantOp>(op, APInt::getZero(width));
      return success();
    }
  ...
  // If extractAmount >= width, we're extracting all bits, so just return lhs.
  if (bitPosition >= width) {
    replaceOpAndCopyNamehint(rewriter, op, lhs);
    return success();
  }

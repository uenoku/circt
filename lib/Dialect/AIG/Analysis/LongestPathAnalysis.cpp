    ...
    for (auto &[point, state] : visitor->getFanOutResults())
      for (const auto &dataFlow : state) {
        if constexpr (elaborate) {
          ...
          auto *node = ctx.instanceGraph->lookup(name);
          auto *instancePathCach = visitor->getInstancePathCache();
          auto topToRoot = instancePathCach->getRelativePaths(
              visitor->getHWModuleOp(), node);
...
LongestPathAnalysis::getAllPaths(StringAttr moduleName,
                                 SmallVectorImpl<DataflowPath> &results,
                                 bool elaboratedPaths) const {
  if (failed(getClosedPaths(moduleName, results, elaboratedPaths)))
    return failure();

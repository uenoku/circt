...
void MacroInstanceOp::getValues(SmallVectorImpl<Value> &values,
  ...
  // Resize the values array to match the number of ports.
  values.resize(mpi.size());

  // Map inputs to their port positions.
  size_t nextInput = 0;
  for (auto &port : mpi.getInputs())
    values[port.argNum] = getInputs()[nextInput++];

  // Map results to their port positions.
  size_t nextResult = 0;
  for (auto &port : mpi.getOutputs())
    values[port.argNum] = getResults()[nextResult++];
}
...
void MacroInstanceOp::print(OpAsmPrinter &p) {
  ...
  p.printOptionalAttrDict(
      (*this)->getAttrs(),
      /*elidedAttrs=*/{"instanceName", "macroName", "moduleNames", "argNames",
                       "resultNames"});
}

...
private:
  ...
  mlir::ParseResult
  parseEntry(StringRef entry, StringRef &filePath, uint32_t &lineNum,
             SmallVectorImpl<std::pair<StringRef, uint32_t>> &columns);
...
void VerilogIndex::parseSourceLocation(StringRef toParse) {
  ...
  for (auto cur : llvm::split(toParse, ", ")) {
    ...
    auto filePathMaybeEmpty = parts[0].trim();

    if (!filePathMaybeEmpty.empty())
      filePath = filePathMaybeEmpty;

    auto line = parts[1].trim();
    auto column = parts[2].trim();
...
mlir::ParseResult VerilogIndex::parseEntry(
    StringRef entry, StringRef &filePath, uint32_t &lineNum,
    SmallVectorImpl<std::pair<StringRef, uint32_t>> &columns) {
  ...
  auto trimmed = parts[0].trim();
  if (!trimmed.empty())
    filePath = trimmed;
  if (parts[1].getAsInteger(10, lineNum))
    return failure();

  SmallVector<StringRef, 3> columnStrs;
  auto columnStr = parts[2].trim();
  if (columnStr.starts_with('{') && columnStr.ends_with('}')) {
    columnStr = columnStr.drop_front().drop_back();
    columnStr.split(columnStrs, ',');
  } else {
    columnStrs.push_back(columnStr);
  }

  for (auto column : columnStrs) {
    uint32_t colNum;
    if (column.getAsInteger(10, colNum))
      return failure();
    columns.emplace_back(column, colNum);
  }

...
class InstancePathElement:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "InstancePathElement":
        ...
        return cls(instance_name=data["instance_name"], module_name=data["module_name"])
...
class Object:
    ...
    def __str__(self) -> str:
        ...
        """
        path = "/".join(
            f"{elem.module_name}:{elem.instance_name}" for elem in self.instance_path
        )
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Object":
        ...
        return cls(
            instance_path=instance_path, name=data["name"], bit_pos=data["bit_pos"]
        )
...
class DataflowPath:
    ...
    def to_flamegraph(self) -> str:
        ...
        fan_in_hierarchy = self._build_hierarchy_string(self.path.fan_in, prefix)
        ...
        for debug_point in self.path.history[::-1]:
            history_hierarchy = self._build_hierarchy_string(debug_point.object, prefix)
        ...
        if current_delay != self.path.delay:
            final_delay = self.path.delay - current_delay

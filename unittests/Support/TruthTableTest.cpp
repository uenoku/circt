...
TEST(TruthTableUtilTest, ExpandTruthTableToInputSpaceSparse) {
  ...
  APInt expanded =
      circt::detail::expandTruthTableToInputSpace(tt, mapping, 3);
...
TEST(TruthTableUtilTest, ExpandTruthTableToInputSpaceGenericPath) {
  ...
  APInt expanded =
      circt::detail::expandTruthTableToInputSpace(tt, mapping, 7);

  APInt expected(128, 0);
  for (unsigned expandedIdx = 0; expandedIdx < 128; ++expandedIdx) {
    unsigned origIdx = ((expandedIdx >> 0) & 1U) | (((expandedIdx >> 3) & 1U) << 1) |
                       (((expandedIdx >> 6) & 1U) << 2);
    if (tt[origIdx])

...
TEST(BinaryTruthTableTest, InputNegation) {
  ...
  EXPECT_EQ(result10, output1); // g(1,0) = 1 (not 0)
...
TEST(NPNClassTest, ConstantFunctions) {
  // Test constant 0 function
  BinaryTruthTable const0(2, 1);
  // All outputs are already 0 by default

  NPNClass canonical0 = NPNClass::computeNPNCanonicalForm(const0);
  EXPECT_EQ(canonical0.truthTable.table.getZExtValue(), 0u);

  // Test constant 1 function
  BinaryTruthTable const1(2, 1);
  const1.setOutput(APInt(2, 0), APInt(1, 1));
  const1.setOutput(APInt(2, 1), APInt(1, 1));
  const1.setOutput(APInt(2, 2), APInt(1, 1));
  const1.setOutput(APInt(2, 3), APInt(1, 1));

...
//===----------------------------------------------------------------------===//
// MiniSATSolver - Bookmark/Rollback for incremental solving
//===----------------------------------------------------------------------===//

TEST(MiniSATSolverTest, BookmarkRollbackBasic) {
  MiniSATSolver s;

  // Add base problem: x1 OR x2
  s.add(1);
  s.add(2);
  s.add(0);

  // Bookmark at decision level 0
  EXPECT_FALSE(s.hasBookmark());
  s.bookmark();
  EXPECT_TRUE(s.hasBookmark());

  // Add temporary variables and clauses
  s.add(3);
  s.add(4);
  s.add(0);
  s.add(-3);
  ...
  // After rollback, original problem should still be SAT
  EXPECT_EQ(s.solve(), MiniSATSolver::kSAT);
  // Only variables 1 and 2 should exist
  EXPECT_EQ(s.val(1), 1); // or -1, but should be valid
}

TEST(MiniSATSolverTest, LearnedClausesPersist) {
  MiniSATSolver s;

  // Build a problem that will generate learned clauses
  // x1 => x2, x2 => x3, x3 => x4
  s.add(-1);
  s.add(2);
  s.add(0);
  ...
  // Solve with x1=true, x4=false (UNSAT, will learn clauses)
  s.assume(1);
  s.assume(-4);
  auto result = s.solve();
  EXPECT_EQ(result, MiniSATSolver::kUNSAT);

  // Add more temporary structure
  s.add(5);
  s.add(6);
  s.add(0);

  // Rollback - learned clauses should persist
  s.rollback();

  // The same query should still be UNSAT (benefiting from learned clauses)
  s.assume(1);
  s.assume(-4);
  result = s.solve();
  EXPECT_EQ(result, MiniSATSolver::kUNSAT);
...
TEST(MiniSATSolverTest, VSIDSWarmthPreserved) {
  MiniSATSolver s;

  // Create a problem with several variables
  for (int i = 1; i <= 5; i++) {
    s.add(i);
    s.add(i + 1);
    s.add(0);
  ...
  // Solve again to modify activity
  s.solve();

  // Rollback
  s.rollback();

  // Should still be able to solve (VSIDS heap intact)
  EXPECT_EQ(s.solve(), MiniSATSolver::kSAT);
}

TEST(MiniSATSolverTest, WatchListIntegrity) {
  MiniSATSolver s;

  // Add base clauses with watches
  s.add(1);
  s.add(2);
  s.add(3);
  s.add(0);
  ...
  // Original problem should still work (watch lists valid)
  EXPECT_EQ(s.solve(), MiniSATSolver::kSAT);
}

TEST(MiniSATSolverTest, MultipleRollbacks) {
  MiniSATSolver s;
  ...
  // First bookmark/rollback cycle
  s.bookmark();
  s.add(3);
  s.add(0); // x3 must be true
  EXPECT_EQ(s.solve(), MiniSATSolver::kSAT);
  s.rollback();

  // Second bookmark/rollback cycle
  s.bookmark();
  s.add(4);
  s.add(5);
  s.add(0);
  ...
  // Original problem should still be SAT
  EXPECT_EQ(s.solve(), MiniSATSolver::kSAT);
}

TEST(MiniSATSolverTest, BookmarkWithUnitClauses) {
  MiniSATSolver s;

  // Add unit clause at root level
  s.add(1);
  s.add(0);

  // Solve to propagate (trail will have x1=true)
  EXPECT_EQ(s.solve(), MiniSATSolver::kSAT);

  // Bookmark (trail should be preserved)
  s.bookmark();
  ...
  // x1 should still be true from unit propagation
  EXPECT_EQ(s.solve(), MiniSATSolver::kSAT);
  EXPECT_EQ(s.val(1), 1);
}

TEST(MiniSATSolverTest, FRAIGStyleUsage) {
  // Simulate FRAIG usage pattern: global solver with multiple cones
  MiniSATSolver s;

  // Shared base logic (e.g., primary inputs)
  s.add(1);
  s.add(2);
  s.add(3);
  s.add(0); // At least one input true

  // Equivalence class 1: verify if node 10 ≡ node 11
  s.bookmark();
  {
    // Add Tseitin for cone of node 10 (10 = 1 AND 2)
    s.add(-10);
    s.add(1);
    s.add(0);
    s.add(-10);
    ...
    // Add Tseitin for cone of node 11 (11 = 1 AND 2) - should be equivalent
    s.add(-11);
    s.add(1);
    s.add(0);
    s.add(-11);
    ...
    // Check: can 10=1 and 11=0?
    s.assume(10);
    s.assume(-11);
    EXPECT_EQ(s.solve(), MiniSATSolver::kUNSAT); // Should be equivalent

    // Check: can 10=0 and 11=1?
    s.assume(-10);
    s.assume(11);
    EXPECT_EQ(s.solve(), MiniSATSolver::kUNSAT); // Should be equivalent
  }
  ...
  // Equivalence class 2: verify if node 20 ≡ node 21
  s.bookmark();
  {
    // Add Tseitin for cone of node 20 (20 = 2 OR 3)
    s.add(20);
    s.add(-2);
    s.add(0);
    s.add(20);
    ...
    // Add Tseitin for cone of node 21 (21 = 2 OR 3) - should be equivalent
    s.add(21);
    s.add(-2);
    s.add(0);
    s.add(21);
  ...
  // After all rollbacks, base problem should still work
  EXPECT_EQ(s.solve(), MiniSATSolver::kSAT);
}

TEST(MiniSATSolverTest, RollbackPreservesClauseCount) {
  MiniSATSolver s;
  ...
  int initialClauses = s.getNumClauses();
  ...
  EXPECT_GT(s.getNumClauses(), initialClauses);

  s.rollback();

  // Should return to initial clause count
  EXPECT_EQ(s.getNumClauses(), initialClauses);
}

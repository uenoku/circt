...
om.class @Foo_Class() -> (test: i1) {
  ...
  auto result =
      evaluator.instantiate(StringAttr::get(&context, "Foo_Class"), {});

  // This test should succeed. There's no actual cycle - just nested
  // field accesses. The issue was that assertions were being evaluated
  // too early, before the worklist drained, causing null pointer access.
  ASSERT_TRUE(succeeded(result));

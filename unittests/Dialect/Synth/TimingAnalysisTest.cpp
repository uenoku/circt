...
const char *ccsPilotVectorInterpolationIR = R"MLIR(
    module attributes {
      synth.liberty.library = {name = "dummy", time_unit = "1ns"},
      synth.nldm.time_unit = #synth.nldm_time_unit<1000.0>,
      synth.ccs.pilot.waveform_delay = true
...
const char *ccsPilotVectorGridInterpolationIR = R"MLIR(
    module attributes {
      synth.liberty.library = {name = "dummy", time_unit = "1ns"},
      synth.nldm.time_unit = #synth.nldm_time_unit<1000.0>,
      synth.ccs.pilot.waveform_delay = true
      ...
      hw.module private @BUF(
        ...
        out Y : i1 {synth.liberty.pin = {
          ...
          synth.ccs.pilot.arcs = [
            #synth.ccs_pilot_arc<"A", "Y", [], [], [], [], [0.0 : f64, 0.0 : f64, 1.0 : f64, 1.0 : f64], [0.0 : f64, 1.0 : f64, 0.0 : f64, 1.0 : f64], [0.0 : f64, 0.0 : f64, 0.0 : f64, 0.0 : f64], [[0.0 : f64, 0.0 : f64], [0.0 : f64, 0.8 : f64], [0.0 : f64, 0.4 : f64], [0.0 : f64, 1.2 : f64]], [[0.0 : f64, 1.0 : f64], [0.0 : f64, 1.0 : f64], [0.0 : f64, 1.0 : f64], [0.0 : f64, 1.0 : f64]], [], [], [], [], []>
          ]
        }}) {
        hw.output %A : i1
      }
...
TEST_F(TimingAnalysisTest, CCSPilotVectorSetInterpolatesAcrossSelectors) {
  OwningOpRef<ModuleOp> module =
  ...
  auto delay = model->computeDelay(ctx);
  EXPECT_EQ(delay.delay, 350);
  EXPECT_NEAR(delay.outputSlew, 400.0, 1e-9);

  SmallVector<WaveformPoint> inputWaveform = {{0.0, 0.0}, {1.0, 1.0}};
  SmallVector<WaveformPoint> outputWaveform;
  EXPECT_TRUE(model->computeOutputWaveform(ctx, inputWaveform, outputWaveform));
  ASSERT_EQ(outputWaveform.size(), 2u);
  EXPECT_NEAR(outputWaveform[0].time, 450.0, 1e-9);
...
TEST_F(TimingAnalysisTest, CCSPilotVectorGridUsesStructuredBilinearBlend) {
  ...
  auto delay = model->computeDelay(ctx);
  EXPECT_EQ(delay.delay, 394);
  EXPECT_NEAR(delay.outputSlew, 630.0, 1e-9);
